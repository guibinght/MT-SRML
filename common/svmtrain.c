...
void exit_with_help()
{
	...
	"	7 -- Polynomial M-norm kernel for doublets\n"
	"	8 -- RBF M-norm kernel for doublets\n"
	"	9 -- Polynomial M-norm kernel for triplets\n"
	"	10 -- RBF M-norm kernel for triplets\n"
	...
	for(i=1;i<argc;i++)
	{
		...
		switch(argv[i-1][1])
		{
			...
			case 'w':
				...
				break;
			case 'f':
				param.binary_classification=atoi(argv[i]);
				break;
			default:
...
int read_problem_dense(const mxArray *label_vec, const mxArray *instance_mat)
{
	...
	sc = (int)mxGetN(instance_mat); //the dimension of instance
	prob.dimension=sc;

	elements = 0;
	// the number of instance
	prob.l = (int)mxGetM(instance_mat); //the number of instance
	label_vector_row_num = (int)mxGetM(label_vec); //the number of instance (the length of label vector)
	//mexPrintf("sc=%d,prob.l=%d,label_vector_row_num=%d\n",sc,prob.l,label_vector_row_num);
	...
	for(i = 0; i < prob.l; i++)
	{
		...
		for(k = 0; k < sc; k++)
		{
			...
			{
				x_space[j].index = k + 1;
				x_space[j].value = samples[k * prob.l + i]; //the ith instance, the kth dimension
				j++;
			}
		}
		x_space[j++].index = -1;
		...
		int nrhs, const mxArray *prhs[] )
{
	...
	// prhs[0]: label vector; prhs[1]: instance matrix
	srand(1);
